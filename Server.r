...
rating.fullpath <- "DataMiningProject/ratings.csv"
movie.fullpath <- "DataMiningProject/movies.csv"
...
shinyServer(function(input, output) {
  ...
  classification.table1 <- classification.table1[, list(avg_score=mean(rating), num_genre_movies = sum(counter), movie_list = list(movieId)), by = c('genres','userId')]
  ...
  #classification.table1 <- classification.table1[classification.table1$num_genre_movies>1,]
  
  # label user with majority of movie genre watched
  classification.table1 <- classification.table1[ , .SD[which.max(num_genre_movies)], by = c('userId')]
